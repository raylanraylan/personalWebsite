...
<template>
  <section ref="lightingSection" class="w-full h-screen bg-[url(@/assets/desk.jpeg)] bg-cover bg-center bg-no-repeat relative">
    <div class="absolute inset-0 pointer-events-none transition-all duration-75" :style="{background: `radial-gradient(400px at ${lightPosition.x} ${lightPosition.y}, transparent 0%, rgba(0, 0, 0, 0.85) 100%)`}"/>
    <div class="px-[5vw] pt-32 pb-20 max-w-4xl mx-auto flex flex-col relative z-10 text-center">
      <div>
        <h1 class="text-muted-foreground">{{ props.fileNumber }}</h1>
